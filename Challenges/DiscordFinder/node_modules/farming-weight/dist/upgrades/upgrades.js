...
import { UpgradeAction, UpgradeCategory, UpgradeReason } from "../constants/upgrades.js";
...
export function getFortune(level, source) {
    return Math.min(Math.max(level ?? 0, 0), source.maxLevel) * source.fortunePerLevel;
}
export function getItemUpgrades(upgradeable) {
    return [
...
export function getLastItemUpgradeableTo(upgradeable, options) {
    const upgrade = upgradeable.getItemUpgrade();
    if (!upgrade || upgrade.reason === UpgradeReason.Situational)
        return undefined;
    let last = upgrade;
...
export function getSourceProgress(upgradeable, sources, zeroed = false) {
    const result = [];
    ...
    for (const source of sources) {
        ...
        if (source.info) {
            ...
            if (info)
                progress.info = info;
            if (maxInfo)
                progress.maxInfo = maxInfo;
            if (nextInfo)
                progress.nextInfo = nextInfo;
        }
        if (source.wiki) {
...
export function getUpgradeableEnchants(upgradeable) {
    if (!upgradeable.type)
        return [];
    const result = [];
    for (const enchantId in FARMING_ENCHANTS) {
        ...
        if (!enchant || !enchant.appliesTo.includes(upgradeable.type))
            continue;
        // Skip if the enchantment is crop specific and the crop doesn't match
        if (upgradeable.crop && enchant.cropSpecific !== upgradeable.crop)
            continue;
