...
export function getLevelByXp(xp, type) {
    let xpTable;
    switch (type) {
        case 'runecrafting':
    ...
    if (level < maxLevel)
        xpForNext = Math.ceil(xpTable?.[level + 1] || 0);
...
export function getSlayerLevel(slayer) {
    if (!slayer) {
...
export function getTrophyFishRank(level) {
    if (1 === level) {
        return 'Bronze';
    }
    else if (2 === level) {
...
export function getBestiaryLevel(userProfile) {
...
export function getSlayer(data) {
    if (!data?.slayer?.slayer_bosses)
        return null;
    return {
...
function getDungeonsFloor(data, type, floor) {
    return {
        fastestRun: (data?.dungeons?.dungeon_types?.[type]?.best_runs?.[floor] || [])?.sort((a, b) => a?.elapsed_time - b?.elapsed_time)[0],
...
export function getDungeons(data) {
    return {
...
export function getJacobData(data) {
    return {
        medals: {
...
export function getChocolateFactory(data) {
...
export function getCrimsonIsle(data) {
    return {
        ...
        trophyFish: {
            ...
            caught: {
                ...
                bronze: Object.keys(data?.nether_island_player_data?.trophy_fish || {}).filter((key) => key.endsWith('_bronze'))
                    .length || 0,
                silver: Object.keys(data?.nether_island_player_data?.trophy_fish || {}).filter((key) => key.endsWith('_silver'))
                    .length || 0,
                gold: Object.keys(data?.nether_island_player_data?.trophy_fish || {}).filter((key) => key.endsWith('_gold'))
                    .length || 0,
                diamond: Object.keys(data?.nether_island_player_data?.trophy_fish || {}).filter((key) => key.endsWith('_diamond'))
                    .length || 0
            }
        },
        dojo: {
            belt: getBelt(Object.keys(data?.nether_island_player_data?.dojo ?? {})
